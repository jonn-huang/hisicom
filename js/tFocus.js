...
function Focus() {
	...
	function bytag(tag, obj) {
		return (typeof obj == 'object' ? obj : byid(obj))
	...
	byid('markLeft').onmouseover = byid('prev').onmouseover = function() {
		doMove(byid('prev'), 'opacity', 100);
	}
	byid('markLeft').onmouseout = byid('prev').onmouseout = function() {
		doMove(byid('prev'), 'opacity', 0);
	}
	byid('markRight').onmouseover = byid('next').onmouseover = function() {
		doMove(byid('next'), 'opacity', 100);
	}
	byid('markRight').onmouseout = byid('next').onmouseout = function() {
		doMove(byid('next'), 'opacity', 0);
	}
	function changePic() {
		for ( var i = 0; i < oPicLis.length; i++) {
		...
		if (iActive == 0) {
			...
			doMove(bytag('ul', oBtn)[0], 'left', -(oPicLis.length - 4)
					* (oBtnLis[0].offsetWidth + 14));
		} else {
			doMove(bytag('ul', oBtn)[0], 'left', -(iActive - 1)
					* (oBtnLis[0].offsetWidth + 14));
		}
	}
	;
